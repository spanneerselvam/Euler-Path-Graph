...
//code to check if graph has euler path, returns true
...
//in order to have a euler path, graph must have atmost 2 odd degree vertices
       if (counter > 2)  
	return false;
       else
      {
...
//similar to euler cycle except we must start at an odd edge
 std::vector<T> find_euler_path() const
   {
       std::vector<T> ans;
       std::stack<T> S;
       std::size_t counter = 0; 
       ...
       for (auto v: V())
           A[v] = Adj(v);
       // change this so it only starts at a vertex of an odd degree

       // the first vertex in the graph should have an odd degree
       // unless graph has no odd degree vertices 
       for (auto e: V())
       {
           if (deg(e) % 2 != 0)
	   {
